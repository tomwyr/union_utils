...
class UnionBuildConfigParser {
  ...
  UnionBuildConfig _parseConfig(Map<String, dynamic> config) =>
      UnionBuildConfig(
  ...
  UnionUtilities _parseUtilities(dynamic config) {
    ...
    final utilities = UnionUtilities(
      ...
      mapOrNull:
          localConfig.remove('map_or_null') as bool? ?? defaults.mapOrNull,
      asType: localConfig.remove('as_type') as bool? ?? defaults.asType,
      asTypeOrNull: localConfig.remove('as_type_or_null') as bool? ??
          defaults.asTypeOrNull,
    );
